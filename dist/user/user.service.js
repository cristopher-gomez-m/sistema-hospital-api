"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserService = void 0;
const common_1 = require("@nestjs/common");
const user_repository_1 = require("./user.repository");
const typeorm_1 = require("@nestjs/typeorm");
const user_entity_1 = require("./entities/user.entity");
const typeorm_2 = require("typeorm");
const historial_clinico_entity_1 = require("../historial-clinico/entities/historial-clinico.entity");
const historial_clinico_service_1 = require("../historial-clinico/historial-clinico.service");
let UserService = exports.UserService = class UserService {
    constructor(userRepository, historialService) {
        this.userRepository = userRepository;
        this.historialService = historialService;
    }
    async create(createUserDto) {
        const usuario = this.userRepository.create(createUserDto);
        const historialClinico = new historial_clinico_entity_1.HistorialClinico();
        await this.historialService.save(historialClinico);
        usuario.historial_clinico = historialClinico;
        await this.userRepository.save(usuario);
        return usuario;
    }
    findByUsername(email) {
        return this.userRepository.find({
            relations: ['rol'],
            where: { email },
        });
    }
    findById(id) {
        return this.userRepository.findOne({
            relations: ['rol', 'historial_clinico'],
            where: { id },
        });
    }
    findMedicoById(id) {
        return this.userRepository.findOne({
            where: { id },
        });
    }
    findAllPacientes() {
        return this.userRepository.find({
            where: { rol: (0, typeorm_2.Equal)(1) },
        });
    }
    findAllMedicos() {
        return this.userRepository.find({
            where: { rol: (0, typeorm_2.Equal)(2) },
        });
    }
    async findUsuariosSinConsultorio(consultorioIds) {
        return await this.userRepository.find({
            where: {
                id: (0, typeorm_2.Not)((0, typeorm_2.In)(consultorioIds)),
                rol: (0, typeorm_2.Equal)(2),
            },
        });
    }
    findOneByUsername(email) {
        return this.userRepository.findOne({
            relations: ['rol'],
            where: { email }
        });
    }
    async remove(id) {
        const user = await this.userRepository.findOne({
            relations: ['historial_clinico'],
            where: { id }
        });
        if (!user) {
            throw new common_1.NotFoundException('Usuario no encontrado');
        }
        await this.userRepository.remove(user);
        await this.historialService.remove(user.historial_clinico.id);
    }
    async updateNombreYApellido(user_id, updateUserDto) {
        const user = await this.userRepository.findOne({
            where: { id: user_id }
        });
        user.nombre = updateUserDto.nombre;
        user.apellido = updateUserDto.apellido;
        const updatedUser = await this.userRepository.save(user);
        return updatedUser;
    }
    async updateMedico(id, updateMedicoDto) {
        return await this.userRepository.update(id, updateMedicoDto);
    }
};
exports.UserService = UserService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(user_entity_1.User)),
    __metadata("design:paramtypes", [user_repository_1.UserRepository,
        historial_clinico_service_1.HistorialClinicoService])
], UserService);
//# sourceMappingURL=user.service.js.map